...
| --- | --- | --- |
| `ContractFirstDatasetLocator` | Default; prefer `servers` and schema objects (`schema_` with `physicalName`/`name`) from the contract to resolve `format`, `path`/`table`, and optional custom properties (for example, Delta time-travel hints). Falls back to caller-provided overrides. | Use a contract’s Unity Catalog coordinates (`catalog.schema.table`) automatically when the caller passes only a contract reference. |
