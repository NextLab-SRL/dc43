...
def _pipeline_example_script(state: Mapping[str, Any]) -> str:
    ...
    for key, value in selected_raw.items():
        key_text = _clean_str(key) or str(key)
    ...
    for key, value in configuration_raw.items():
        if isinstance(value, Mapping):
            configuration[str(key)] = value

    integration_key = selected.get("pipeline_integration", "") or ""
    integration_config = configuration.get("pipeline_integration", {})
    ...
    def module_summary_line(module_key: str, option_label: str) -> str:
        module_config = configuration.get(module_key, {})
        details: List[str] = []
        if isinstance(module_config, Mapping):
            for field_name in sorted(module_config.keys()):
                raw_value = module_config[field_name]
                if isinstance(raw_value, Mapping):
                    nested: Dict[str, str] = {}
                    for nested_key, nested_value in raw_value.items():
                        nested_name = _clean_str(nested_key) or str(nested_key)
                        nested_text = _clean_str(nested_value)
                ...
                if isinstance(raw_value, (list, tuple, set)):
                    ...
                    continue
                value_text = _clean_str(raw_value)
        ...
        option_text = option_label or "unspecified"
        detail_text = ", ".join(details) if details else "no explicit settings"
        return f"- {module_key} ({option_text}): {detail_text}"

    module_summaries = [
        module_summary_line(module_key, option_value)
        for module_key, option_value in sorted(selected.items())
    ]
    ...
    import_parts = ["load_backends"]
    if integration_key == "spark":
        import_parts.append("build_spark_context")
    elif integration_key == "dlt":
        import_parts.append("build_dlt_context")
    lines.append(f"from bootstrap_pipeline import {', '.join(import_parts)}")
    lines.append("")

    has_contracts = bool(selected.get("contracts_backend"))
    has_products = bool(selected.get("products_backend"))
    has_data_quality = bool(selected.get("data_quality"))
    has_governance_store = bool(selected.get("governance_store"))
    ...
    integration_literal = json.dumps(integration_key)
    spark_runtime_literal = json.dumps(spark_runtime) if spark_runtime else "None"
    spark_workspace_literal = json.dumps(spark_workspace_url) if spark_workspace_url else "None"
    spark_profile_literal = json.dumps(spark_workspace_profile) if spark_workspace_profile else "None"
    spark_cluster_literal = json.dumps(spark_cluster) if spark_cluster else "None"
    dlt_workspace_literal = json.dumps(dlt_workspace_url) if dlt_workspace_url else "None"
    dlt_profile_literal = json.dumps(dlt_workspace_profile) if dlt_workspace_profile else "None"
    dlt_pipeline_literal = json.dumps(dlt_pipeline_name) if dlt_pipeline_name else "None"
    dlt_notebook_literal = json.dumps(dlt_notebook_path) if dlt_notebook_path else "None"
    dlt_target_literal = json.dumps(dlt_target_schema) if dlt_target_schema else "None"
    ...
    lines.extend(
        [
            ...
            f"    integration = {integration_literal}",
