...
from tests.helpers.orders import build_orders_contract, materialise_orders
...
def test_governance_wrappers_require_only_governance_client(
    ...
    validation = getattr(_exec_result, 'result', _exec_result[0] if isinstance(_exec_result, tuple) else _exec_result)
    status = getattr(_exec_result, 'status', _exec_result[1] if isinstance(_exec_result, tuple) else None)
...
def test_write_with_governance_violation_blocks_by_default(
    ...
    result = getattr(_exec_result, 'result', _exec_result[0] if isinstance(_exec_result, tuple) else _exec_result)
    status = getattr(_exec_result, 'status', _exec_result[1] if isinstance(_exec_result, tuple) else None)
...
def test_write_with_governance_keeps_existing_link_for_contract_upgrade(
    ...
    result = getattr(_exec_result, 'result', _exec_result[0] if isinstance(_exec_result, tuple) else _exec_result)
    status_ok = getattr(_exec_result, 'status', _exec_result[1] if isinstance(_exec_result, tuple) else None)
...
def test_governance_service_persists_draft_context(spark, tmp_path: Path) -> None:
    ...
    request = GovernanceSparkWriteRequest(
        context=GovernanceWriteContext(
            contract={"contract_id": contract.id, "contract_version": contract.version},
            dataset_id=contract.id,
            dataset_version="handles-run",
...
def test_read_executor_applies_plan_data_product_status(spark) -> None:
    ...
    plan = ResolvedReadPlan(
        ...
        policy=GovernancePolicy(allowed_data_product_statuses=("active", "draft")),
    )
    executor = BatchReadExecutor(
        spark=spark,
        request=GovernanceSparkReadRequest(context=GovernanceReadContext()),
...
def test_write_executor_applies_plan_data_product_status(spark) -> None:
    ...
    executor = BaseWriteExecutor(
        df=df,
        request=GovernanceSparkWriteRequest(context=GovernanceWriteContext()),
        governance_service=None,
        enforce=True,
        auto_cast=True,
        plan=plan,
